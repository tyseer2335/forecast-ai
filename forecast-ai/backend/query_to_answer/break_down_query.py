...
def generate_search_queries(client: any, metric_eval_ranking: ForecastRequest) -> dict:
    """
    Generates search queries based on a forecast question to gather information from various sources.
    
    Args:
        client (any): The client object for making API requests, typically a chatbot or language model interface.
        metric_eval_ranking (ForecastRequest): An instance of ForecastRequest containing parameters for query generation.
        
    Returns:
        dict: A dictionary with search queries categorized by source and a success flag.
    """
    # variables
...
def ensure_response_success(response: list[str], num_of_queries_per_source: dict[str, int],
    ...
    """
    Validates and organizes the generated search queries into their respective sources.
    
    Args:
        response (list[str]): A list of search query strings generated by the model.
        num_of_queries_per_source (dict[str, int]): Number of queries required per source.
        num_of_queries_per_automatic (int): Number of general queries required (without specific sources).
        perc_of_each_source (dict[str, float]): Proportion of queries for each source.

    Returns:
        dict: A dictionary with grouped queries by source and a 'success' flag indicating if requirements are met.
    """
    # Example:
