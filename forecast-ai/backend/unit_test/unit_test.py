...
def unit_test_all():
    try:
        ...
        logging.info(
            "\033[92m[Unit Test] Environment variables loaded successfully.\033[0m"
        )
        request = test_create_request()
        logging.info(
            "\033[92m[Unit Test] ForecastRequest object created successfully.\033[0m"
        )
        search_queries = test_generate_search_queries(client, request)
        logging.info(
            "\033[92m[Unit Test] Search queries generated successfully.\033[0m"
        )
        news = test_collect_news(search_queries, request)
        logging.info("\033[92m[Unit Test] News collected successfully.\033[0m")
        news_with_content = test_scrapping_content(
            news,
            LOCAL_OR_PROD,
            DOCKER_OR_LAMBDATEST,
            SINGLE_OR_PARALLEL,
        ...
        logging.info(
            "\033[92m[Unit Test] News with content scrapped successfully.\033[0m"
        )
        news_objects = test_convert_to_article(news_with_content)
        logging.info(
            "\033[92m[Unit Test] News converted to Article objects successfully.\033[0m"
        )
        test_get_relevance_score(news_objects, request, client)
        logging.info(
            "\033[92m[Unit Test] Relevance score calculated successfully.\033[0m"
        )
        ranked_news_with_content = test_sort_and_filter(news_objects, request)
        logging.info(
            "\033[92m[Unit Test] News sorted and filtered successfully.\033[0m"
        )
        ...
        logging.info(
            "\033[92m[Unit Test Success]\033[0m All tests passed successfully."
        )
...
def test_env_var() -> tuple[OpenAI, str]:
    ...
    LOCAL_OR_PROD = os.getenv(
        "LOCAL_OR_PROD"
    )  # set to `local` or `prod`. use `local` for testing selenium
    ...
    assert isinstance(LOCAL_OR_PROD, str) and (
        LOCAL_OR_PROD == "local" or LOCAL_OR_PROD == "prod"
    )
    assert isinstance(DOCKER_OR_LAMBDATEST, str) and (
        DOCKER_OR_LAMBDATEST == "docker" or DOCKER_OR_LAMBDATEST == "lambdatest"
    )
    assert isinstance(SINGLE_OR_PARALLEL, str) and (
        SINGLE_OR_PARALLEL == "single" or SINGLE_OR_PARALLEL == "parallel"
    )
    assert isinstance(USERNAME, str)
    assert isinstance(ACCESS_KEY, str)
    logging.info(
        f"\033[92m[test_env_var] client: {client}, LOCAL_OR_PROD: {LOCAL_OR_PROD}, DOCKER_OR_LAMBDATEST: {DOCKER_OR_LAMBDATEST}, SINGLE_OR_PARALLEL: {SINGLE_OR_PARALLEL}, USERNAME: {USERNAME}, ACCESS_KEY: {ACCESS_KEY}\033[0m"
    )
    return (
        client,
        LOCAL_OR_PROD,
...
def test_create_request() -> ForecastRequest:
    ...
    request = ForecastRequest(
        question="Who will win the US 2024 election?",
        num_queries=5,
    ...
    assert request.perc_of_each_source == {
        "automatic": 0.6,
        "x.com": 0.2,
        "facebook.com": 0.2,
    }
...
def test_generate_search_queries(client, request: ForecastRequest) -> dict:
    ...
    logging.info(
        f"\033[92m[test_generate_search_queries] search_queries: {search_queries}\033[0m"
    )
...
def test_collect_news(search_queries: dict, request: ForecastRequest) -> dict:
    ...
    assert all(
        isinstance(article, dict) for value in news.values() for article in value
    )
    ...
    assert all(
        "published date" in article for value in news.values() for article in value
    )
    ...
    assert all(
        isinstance(article["publisher"], dict)
...
def test_scrapping_content(
    news: dict,
    LOCAL_OR_PROD: str,
    DOCKER_OR_LAMBDATEST: str,
    SINGLE_OR_PARALLEL: str,
    USERNAME: str,
    ACCESS_KEY: str,
) -> dict:
    """
    ...
    assert all(
        ...
        for value in news_with_content.values()
        for article in value
    )
    assert all(
        "title" in article for value in news_with_content.values() for article in value
    )
    assert all(
    ...
    assert all(
        "published date" in article
        for value in news_with_content.values()
        for article in value
    )
    assert all(
        "url" in article for value in news_with_content.values() for article in value
    )
    assert all(
    ...
    assert all(
        isinstance(article["content"], dict)
        for value in news_with_content.values()
        for article in value
    )
    assert all(
    ...
    assert all(
        "media" in article["content"]
        for value in news_with_content.values()
        for article in value
    )
    logging.info(
        f"\033[92m[test_scrapping_content] news_with_content: {news_with_content}\033[0m"
    )
...
def test_convert_to_article(news_with_content: dict) -> dict:
    ...
    assert all(
        ...
        for value in news_objects.values()
        for article in value
    )
    logging.info(
        f"\033[92m[test_convert_to_article] news_objects: {news_objects}\033[0m"
    )
    return news_objects


def test_get_relevance_score(
    news_objects: dict, request: ForecastRequest, client: OpenAI
):
    """
    ...
    # update self.score
    assert all(
        isinstance(article.score, float)
        for value in news_objects.values()
        for article in value
    )
    logging.info(
        f"\033[92m[test_get_relevance_score] news_objects: {news_objects}\033[0m"
    )
...
def test_sort_and_filter(news_objects: dict, request: ForecastRequest) -> dict:
    ...
    ranked_news_with_content = filtering.sort_and_filter(
        news_objects, request.after_ranking_num_articles, request.perc_of_each_source
    )
    # {'x.com': [<model.article.Article at 0x22948f6bc50>],
    #  'facebook.com': [<model.article.Article at 0x22948f6b510>],
    ...
    assert all(
        isinstance(article, Article)
        for value in ranked_news_with_content.values()
        for article in value
    )
    logging.info(
        f"\033[92m[test_sort_and_filter] ranked_news_with_content: {ranked_news_with_content}\033[0m"
    )
    return ranked_news_with_content


def test_generate_forecast(
    request: ForecastRequest, client: any, ranked_news_with_content: dict
):
    """
    ...
    assert all(
        isinstance(value, list) for value in answer["answer"]["Sources"].values()
    )
    assert all(
        isinstance(article, dict)
        for value in answer["answer"]["Sources"].values()
        for article in value
    )
