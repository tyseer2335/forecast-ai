...
import {HeaderBar, ViewOnlyHeaderBar} from "./HeaderBar";
import {MainContent, ViewOnlyMainContent} from "./MainContent";
...
const ViewOnlyContainer: React.FC = () => {
  // Similar to MainContainer, but for viewing shared chat
  // This component is used when a user wants to view a shared chat by the link with chatHash

  const db = getFirestore();
  const [chats, setChats] = useState<Chat[]>([]);
  const chatTitle = "Shared Chat";

  // Fetch the chat reference from Firestore using the chatHash
  const fetchChatRef = async () => {
    
    // 1. Get the chatHash from the URL: /view/<chatHash>
    const chatRefHash = window.location.pathname.split("/").pop();

    // 2. Use backend API to get the chat document from the chatHash 
    try {
...
export { MainContainer, ViewOnlyContainer };